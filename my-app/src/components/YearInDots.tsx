  ...
  return (
    ...
    >
      ...
      <span
        key={replayKey}
        aria-hidden="true"
        className="absolute top-[19%] left-[11.5%] z-2 grid w-[77%] grid-cols-[repeat(22,minmax(0,1fr))] gap-1 sm:top-[19.5%] sm:left-[12%] sm:w-[76%] sm:gap-[clamp(4px,1.05vw,8px)]"
      >
        {calendar &&
          Array.from({ length: calendar.totalDays }, (_, index) => {
            ...
            return (
              <span
                key={index}
                className={cn(
                  "year-in-dots-dot aspect-square w-full rounded-full transition-[transform,background-color,box-shadow] duration-300 ease-[cubic-bezier(0.16,1.2,.3,1)]",
                  elapsed
